// ===============================================================
// Levitador neumático - PID con reglas expertas (gain scheduling)
...
// ----- RANGO -----
const float H_MIN = 00.0;   // altura mínima en cm
const float H_MAX = 45.0;  // altura máxima en cm
...
void setup() {
  ...
  Serial.println(F("LEVITADOR: inicio con PID + reglas expertas (gs)"));
...
void loop() {
  ...
  err = setpoint - (altura - 10); // compensación de offset
  integ += err * DT;
  ...
  // ==================================================
  // Reglas expertas suavizadas (gain scheduling)
  // ==================================================
  float gs = 1.0; // factor de ajuste

  if (fabs(err) > 0.8)       gs = 1.0;   // lejos → PID completo
  else if (fabs(err) > 0.4)  gs = 0.85;  // medio → 85% del PID
  else if (fabs(err) > 0.2)  gs = 0.70;  // más cerca → 70%
  else if (fabs(err) > 0.01) gs = 0.55;  // casi en el setpoint
  else                       gs = 0.45;  // muy cerca → muy suave
  u = gs * u_pid;
